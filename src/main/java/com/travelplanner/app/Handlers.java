            ...
            switch (method) {
                case "GET": {
                    ...
                    for (int i = 0; i < locations.length; i++) {
                        ...
                        json.append(
                                String.format(
                                        "{\"id\":\"%s\", \"name\":\"%s\", \"description\":\"%s\", \"price\":%.2f%s}",
                                        escapeJson(loc.getId()), escapeJson(loc.getName()),
                                        escapeJson(loc.getDescription()),
                ...
                case "PUT": {
                    ...
                    if (id != null) {
                        double price = -1; // Default -1 means no update explicitly
                        if (priceStr != null && !priceStr.isEmpty()) {
                            try {
                                price = Double.parseDouble(priceStr);
                            } catch (NumberFormatException ignored) {
                        ...
                        boolean updated = tourList.updateLocation(id, price, imageUrl);
                        if (updated) {
    ...
    public void handle(HttpExchange t) throws IOException {
        ...
        try {
            switch (method) {
                    ...
                    if (searchId != null) {
                        ...
                        if (found != null) {
                            response = String.format(
                                    ...
                                    escapeJson(found.getId()), escapeJson(found.getName()),
                                    escapeJson(found.getPhone()), escapeJson(found.getEmail()));
                        } else {
                        ...
                        for (int i = 0; i < list.size(); i++) {
                            ...
                            sb.append(String.format("{\"id\":\"%s\",\"name\":\"%s\",\"phone\":\"%s\",\"email\":\"%s\"}",
                                    escapeJson(c.getId()), escapeJson(c.getName()), escapeJson(c.getPhone()),
                ...
                case "PUT": {
                    // Update customer info (ID is fixed and cannot be changed)
                    URI uri = t.getRequestURI();
                    Map<String, String> params = queryToMap(uri.getQuery());
                    String id = params.get("id");
                    ...
                    if (id == null || name == null || phone == null || email == null) {
                        response = "{\"success\": false, \"error\": \"Missing required parameters (id, name, phone, email)\"}";
                        statusCode = 400;
                        break;
                    }

                    if (newId != null && !newId.equals(id)) {
                        response = "{\"success\": false, \"error\": \"Customer ID is immutable and cannot be changed.\"}";
                        statusCode = 400;
                        break;
                    }
                    ...
                    boolean updated = customerTree.update(id, name, phone, email);
                    if (updated) {
                        response = "{\"success\": true, \"message\": \"Customer updated\"}";
                    } else {
                        response = "{\"success\": false, \"error\": \"Failed to update customer\"}";
                        statusCode = 500;
                    }
                    break;
                }
